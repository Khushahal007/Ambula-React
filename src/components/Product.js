...
const Product = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const fetchedProducts = response.data.map((product) => ({ ...product, quantity: 0 }));
  ...
  const handleIncrementQuantity = (productId) => {
    const updatedProducts = products.map((product) =>
      product.id === productId && product.quantity < 5 ? { ...product, quantity: product.quantity + 1 } : product
    );
    ...
    <Box py={4}>
      <Box display="flex" justifyContent="flex-end" mb={4}>
        ...
        >
          <Badge borderRadius="full" colorScheme="red">
            {getTotalItems()} {/* Display the total number of items in the cart */}
          </Badge>
        </IconButton>
      </Box>
      ...
      <VStack align="flex-start" spacing={4} mx="auto">
        ...
        {loading ? (
          ...
          <Grid templateColumns="repeat(5, 1fr)" gap={4} justifyContent="flex-start">
